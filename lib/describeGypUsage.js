...
var debug = require("debug")("node-gyp-counter:describeGypUsage");
...
/**
 * Heuristic to determine if record represents a program using node-gyp
 * during install.
 * @param npmRecord - doc from goodDocIndex
 * @returns {boolean}
 */
function isGypRoot(npmRecord) {
  var hasGypFile = npmRecord.gypfile;
  var hasInstallScript = npmRecord.scripts && npmRecord.scripts.install;
  var usesNodeGypOrNan = npmRecord.dependencies &&
    (npmRecord.dependencies['node-gyp'] ||
    npmRecord.dependencies.nan);
  return !!(hasGypFile || hasInstallScript || usesNodeGypOrNan);
}
  ...
  return dbIndexRows
    ...
    .filter(function (dependentsRecord, packageName) {
      if (dependentsRecord.confirmed === false) {
        debug("Broken dependency - key: " + packageName +
                    ", dependents:" +
...
function gypUsageData(dbWithGoodDocIndex, npmDownloadStatDBName) {
  ...
  return dbWithGoodDocIndex.query('goodDocIndex')
    .then(function (docs) {
      ...
      returnValue.top80OfNodeGypDownloads =
        returnValue.sortedGypDependentsDownloadsHighToLow
          .filter(function (downloads) {
          if (this.stopAt0 <= 0) {
            return false;
          }
          this.stopAt0 -= downloads;
          return true;
        }, {
          stopAt0:
            returnValue.allDownloadsOfNodeGypPackagesAndDependents * 0.8
        }).map(function (downloads, projectName) {
          return Immutable.Map({
            downloads: downloads,
            nodeGypRootPackages:
              returnValue.projectGypDependencies.get(projectName)
          });
        });

      returnValue.gypRootNeededFor80percent =
        returnValue.top80OfNodeGypDownloads
          .reduce(function (topRootMap, downloadRoots) {
          return downloadRoots.get("nodeGypRootPackages")
            .reduce(function (topRootMap, topRootName) {
              return topRootMap.update(topRootName,
                function (existingValue) {
                  return (existingValue || 0) + downloadRoots.get("downloads");
                });
            }, topRootMap);
        }, Immutable.Map());
...
function displayGypUsageData(gypUsageDataObject) {
  var outputData = {
    ...
    "List of top node-gyp dependent packages that account for 80% of downloads":
      gypUsageDataObject.top80OfNodeGypDownloads
      .map(function (downloadsAndRoots) {
        return downloadsAndRoots.get("downloads") + " ; " +
          downloadsAndRoots.get("nodeGypRootPackages")
            .toArray().toString();
      }).toJS(),
    "Node-Gyp Root Packages needed for top 80":
      gypUsageDataObject.gypRootNeededFor80percent.toJS(),
